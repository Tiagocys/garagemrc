...
export function ensureRecaptcha(siteKey) {
  ...
  _recaptchaLoadPromise = new Promise((resolve, reject) => {
    if (window.grecaptcha?.execute) { window.grecaptcha.ready(resolve); return; }
    const s = document.createElement("script");
    s.src = `https://www.google.com/recaptcha/api.js?render=${encodeURIComponent(siteKey)}`;
    s.async = true; s.defer = true;
    s.onload = () => { if (!window.grecaptcha) return reject(new Error("grecaptcha not found")); window.grecaptcha.ready(resolve); };
